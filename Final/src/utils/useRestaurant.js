...
export const useRestaurant = (id) => {
    ...
    async function getRestaurantInfo() {
        const data = await fetch("https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=19.2181987&lng=72.9622965&restaurantId="+id);
...
export const useRestaurantMenu = (id) => {
    ...
    async function getRestaurantMenu() {
        ...
        const itemCardsArray = await json?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card?.itemCards;      
